...
const getModules = function(req,res,next){
  models.Student.findOne({number: req.student.number}).then(d =>{
    req.modules = d.modules
    next()
  }).catch(e=>{res.send(e)})
}
const getModuleActivities = async function(element, start_date_time, end_date_time){
  a = await models.Activity.find({module_code: element, 
...
const getActivities = async function(req,res, next){
  ...
  start_date_time = moment.unix(date_time).startOf('day').format('X')
  console.log(start_date_time)
  end_date_time = moment.unix(date_time).endOf("day").format('X')
  console.log(end_date_time)
  let activities = []

  await req.modules.forEach(async element => {
    module_activities = await getModuleActivities(element, start_date_time, end_date_time)
